                                ...
                                <div className="flex flex-col gap-2 pl-2">
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                        <div className="flex items-center gap-2 min-w-0">
                                            ...
                                            {hasThread && (
                                                <div className="flex items-center gap-1 bg-white/5 px-2 py-0.5 rounded border border-white/10" title="جزء من سلسلة">
                                                    <LinkIcon className="w-3 h-3 text-emerald-400" />
                                                    <span className="text-[10px] font-bold text-emerald-300">سلسلة</span>
                                                </div>
                                            )}
                                        </div>
                                    ...
                                    {/* Linkage Context Area (Redesigned) */}
                                    {hasThread && (
                                        <div className="mt-3 bg-slate-950/50 rounded-lg p-2 border border-white/5 flex flex-col gap-1.5">
                                            {parent && (
                                                <div className="flex items-center gap-2 text-[10px] text-slate-400">
                                                    <span className="text-indigo-400 font-bold bg-indigo-500/10 px-1 rounded">رد على:</span>
                                                    <span className="font-mono text-slate-500">[{parent.internalRefNumber || parent.externalRefNumber || '---'}]</span>
                                                    <span className="truncate flex-1 font-semibold">{parent.subject}</span>
                                                </div>
                                            )}
                                            {children.length > 0 && (
                                                <div className="flex items-center gap-2 text-[10px] text-slate-400">
                                                    <span className="text-emerald-400 font-bold bg-emerald-500/10 px-1 rounded">الردود ({children.length}):</span>
                                                    <span className="truncate flex-1 font-semibold">
                                                        أحدث رد: {children[children.length-1].subject}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
