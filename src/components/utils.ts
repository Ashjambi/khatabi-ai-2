...
// @FIX: Added missing sanitizeHTML utility function
...
export const getStatusChip = (status: LetterStatus) => {
    ...
    return React.createElement(
        'div',
        { className: `inline-flex items-center gap-x-2 rounded-md px-2.5 py-1 text-xs font-medium ${style.bg} ${style.text}` },
...
export const getConfidentialityChip = (confidentiality?: ConfidentialityLevel) => {
    ...
    const style = styles[confidentiality];
